...
class SSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        if channel == self.channel and self.window.data.type() == img1.data.type():
            ...
            if img1.is_cuda:
                window = window.cuda(device=torch.device('cuda:2'))
...
class BasLoss(nn.Module):
    ...
    def forward(self, pred, labels_v):
        pred0, pred1, pred2, pred3, pred4, pred5, pred6, pred7 = pred

        loss0 = ce_ssim_loss(pred0, labels_v)
        loss1 = ce_ssim_loss(pred1, labels_v)
        loss2 = ce_ssim_loss(pred2, labels_v)
        loss3 = ce_ssim_loss(pred3, labels_v)
        loss4 = ce_ssim_loss(pred4, labels_v)
        loss5 = ce_ssim_loss(pred5, labels_v)
